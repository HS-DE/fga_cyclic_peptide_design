...
| --- | --- | --- | --- | --- | --- |
| full_length_1_866 | 1 | 866 | 866 | False | record |
| extracellular_20_866 | 20 | 866 | 847 | True | secondary |
| chain_36_866 | 36 | 866 | 831 | True | preferred |
...
| --- | --- | --- | --- | --- | --- |
| Patch_A | native_visible_surface | A,D,G,J | 18 | high | medium |
| Patch_B | N_terminal_main_chain_surface | A,D,G,J | 18 | medium | medium |
| Patch_C | C_terminal_alphaC_exploratory | A,D,G,J | 18 | exploratory | high |
...
真实候选必须经过复合物预测或 docking 评分后才能进入最终排序。评分字段包括 ipTM/pTM、interface PAE、peptide pLDDT、interface contacts、peptide-target minimum distance、patch consistency、multi-seed pose consistency 和 Cys-Cys geometry。当前真实 raw candidates 数: 437；真实 scored candidates 数: 0。

## 10. 负筛选规则

负筛选目标包括 ALB、APOA1、TF、A2M、C3、IGG_FC。负筛选只用于降低明显非特异 sticky peptide 风险，不能证明绝对特异。当前负筛选结果行数: 0。
...
当前尚未运行真实生成/复合物预测模型，或尚未完成负筛选，因此没有可用于合成的真实候选序列。
